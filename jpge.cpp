...
namespace jpge
{
	...
	template<class T> static void RGB_to_YCC_GPU(image *img, const T *src, int width, int height, int bpp)
	{
		try
		{
			...
			queue.enqueueWriteBuffer(bufSrc, CL_TRUE, 0, sizeof(unsigned char) * width * height * bpp, src);
			...
			rgb_to_ycc_kernel.setArg(0, bufY);
			rgb_to_ycc_kernel.setArg(1, bufC1);
			rgb_to_ycc_kernel.setArg(2, bufC2);
			rgb_to_ycc_kernel.setArg(3, bufSrc);
...
#define JPGE_PUT_BYTE(c) { *m_pOut_buf++ = (c); if (--m_out_buf_left == 0) flush_output_buffer(); }
	...
	bool jpeg_encoder::read_image(const uint8 *image_data, int width, int height, int bpp)
	{
		...
		{
			load_mcu_YCC_GPU(image_data, width, height, bpp);
		}
